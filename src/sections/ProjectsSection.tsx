...
const ProjectCard: React.FC<ProjectCardProps> = ({ project, index, scrollYProgress }) => {
  ...
  const scale = useTransform(scrollYProgress, [index / TOTAL_CARDS, 1], [1, targetScale])

  const cardRadius = 'rounded-[40px] sm:rounded-[50px] md:rounded-[60px]'
  const imgRadius = 'rounded-[24px] sm:rounded-[32px] md:rounded-[40px]'

  return (
    <div className="h-[85vh] flex items-start" style={{ top: 0 }}>
      <motion.div
        className={`sticky top-24 md:top-32 w-full border-2 border-[#D7E2EA] p-4 sm:p-6 md:p-8 ${cardRadius}`}
        style={{
          background: '#0C0C0C',
          scale,
          top: `${96 + index * 28}px`,
